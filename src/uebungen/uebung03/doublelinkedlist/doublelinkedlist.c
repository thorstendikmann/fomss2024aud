...
{
    printf("[%c | %p | <- %p | -> %p]", *(n->value), (void *)n, (void *)n->prev, (void *)n->next);
}
...
node *doublelinkedlist_insert_after(DoubleLinkedList *l, node *item, char value)
{
    ...
    *(new_item->value) = value;
    ...
    item->next->prev = new_item; // Setting old item->next
    new_item->prev = item;
    item->next = new_item;
...
{
    // l is not even needed here!
    // Fast implementation due to DoubleLinkedList
    return item->prev;
}
#if defined(__GNUC__) || defined(__clang__)
